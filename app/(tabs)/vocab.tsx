...
import { deleteFromVocabulary, getVocabulary, saveToVocabulary, VocabularyItem } from '@/services/supabase/vocabulary';
...
export default function VocabScreen() {
    ...
    const [primaryPracticeItem, setPrimaryPracticeItem] = useState<VocabularyItem | null>(null);
    ...
    const showActionMenu = (item: VocabularyItem) => {
        Alert.alert(
            ...
            [
                ...
                { text: "Delete", style: "destructive", onPress: () => item.id && handleDelete(item.id) },
                { text: "Cancel", style: "cancel" }
            ]
        );
    };
        ...
        <SafeAreaView className="flex-1 bg-white" edges={['top']}>
            ...
            <View className="px-6 mb-4 flex-row gap-3">
                ...
                <TouchableOpacity
                    ...
                    className="h-12 px-5 bg-blue-500 rounded-2xl flex-row items-center gap-2 shadow-sm shadow-blue-200"
                >
                        ...
                        <View className="mb-4 bg-white rounded-[24px] border border-gray-100 p-5 shadow-sm shadow-gray-50">
                            <View className="flex-row justify-between items-start">
                                <View className="flex-1 mr-4">
                                    ...
                                    <Text className="text-lg font-bold text-gray-900 leading-snug">{item.phrase}</Text>
                                    {item.translation && (
                                        <Text className="text-gray-500 font-medium italic mt-1">&quot;{item.translation}&quot;</Text>
                                    )}
                                </View>
                    ...
                    ListEmptyComponent={
                        <View className="items-center mt-20 opacity-40">
                            ...
                            <Text className="text-gray-500 font-bold mt-4 text-center px-10">
                                {searchQuery ? "No matches found" : "Your vocabulary is empty.\nAdd phrases to start learning!"}
                            </Text>
                        </View>
                    }
                />
                ...
                >
                    <View className="flex-1 px-6 pt-6">
                        <View className="flex-row justify-between items-center mb-6">
                            <Text className="text-2xl font-bold text-gray-900">Add Phrase</Text>
                            <TouchableOpacity
                        ...
                        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                            <View className="mb-6">
                                <Text className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">Language</Text>
                                <TouchableOpacity
                                ...
                                >
                                    ...
                                    <Text className="text-lg font-semibold text-gray-900 flex-1">{newLanguage.name}</Text>
                                    <Text className="text-blue-500 font-bold text-sm">Change</Text>
                                </TouchableOpacity>
                            </View>

                            <View className="mb-6">
                                <Text className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">Phrase *</Text>
                                <TextInput
                            ...
                            <View className="mb-8">
                                <Text className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">Translation (Optional)</Text>
                                <TextInput
                                    className="bg-gray-50 p-4 rounded-2xl text-lg border border-gray-100"
                            ...
                            <TouchableOpacity
                                ...
                                className={`w-full h-14 rounded-full items-center justify-center shadow-lg shadow-blue-200 ${!newPhrase.trim() ? 'bg-gray-200' : 'bg-blue-500'
                                    }`}
            ...
            <Modal visible={isMultiSelectVisible} animationType="slide" presentationStyle="pageSheet">
                <View className="flex-1 bg-white">
                    <View className="px-6 py-4 border-b border-gray-100 bg-white z-10">
                        ...
                        <Text className="text-gray-500 text-sm">Select additional phrases to include in your conversation</Text>
                    </View>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => {
                            ...
                            return (
                                ...
                                >
                                    <View className="flex-1 mr-4">
                                        <Text className={`font-bold text-base ${isSelected || isPrimary ? 'text-gray-900' : 'text-gray-600'
                                            }`}>{item.phrase}</Text>
                                        <Text className="text-gray-400 text-xs">{item.translation}</Text>
                                    </View>
