...
export async function configureAudioSession(): Promise<void> {
    ...
    try {
        ...
        Logger.info(TAG, `[DIAG] Setting audio session options: ${JSON.stringify(options)}`);
        ...
        Logger.info(TAG, `[DIAG] setAudioSessionActivity(true) resolved in ${Date.now() - activateStart}ms`);
        ...
        isConfigured = true;
        Logger.info(TAG, '[DIAG] iOS audio session fully configured and activated (session management enabled)');
    } catch (error) {
        Logger.error(TAG, '[DIAG] Failed to configure audio session', error);
        Logger.warn(TAG, `[DIAG] Error details: ${error instanceof Error ? error.stack : String(error)}`);
    }
}
