...
//
// This serves the web fallback page when a share link is opened in a browser.
// Cloudflare Workers correctly serve text/html (unlike Supabase Edge Functions
// on the standard supabase.co domain, which rewrite text/html to text/plain).
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      // Query Supabase REST API directly — no external SDK import needed
      ...
      const rows = (await res.json()) as SharedScenarioRow[];
